...
grade levels: Grade K, 3rd, 6th, 8th, 9th, 10th grade.
...
3. some districts in the shape file do not appear in the ospi data (shows in "match district names with shape file" section)

* data for comparison among ethnicity groups was downloaded directly from OSPI website
* data for comparison among income groups was downloaded and scraped for each district and manually compiled
...
df_nonpass_race <- df_course_outcomes_by_outcome %>%
...
df_nonpass_race_1819 <- filter(df_nonpass_race, `School Year` != "2020-21") 
colnames(course_geo_1819)[5] <- "SY2018-19"
  
df_nonpass_race_2021 <- filter(df_nonpass_race, `School Year` == "2020-21")
...
plot_spatial_year_diff_white <- ggplot() +
  ...
  scale_fill_gradient2(low = "#e8e3d3", high = "#4b2e83") +
  theme(panel.background = element_blank(),
        legend.title = element_text(size = 10),
        plot.title = element_text(size = 14),
        plot.subtitle = element_text(size = 10),
        plot.caption = element_text(hjust = 0, size = 7, face = "italic")) +
  labs(title="Difference in all non-passing grade from SY2018-19 to SY2020-21",
...
```{r A.3.2.1.ii The Difference in all non-passing grade from 2018-19 to 2020-21 (low-income and non-low income), message=FALSE}
...
wa_nonpass_inc <- left_join(df_nonpass_inc, wa, by = c("District" = "NAME"), copy=TRUE) 
...
# ggsave("/Users/alexliu/Desktop/A.3.2.1.ii nonpass diff year low income.png")

wa_nonpass_inc %>% 
  filter(StudentGroup == "Non-Low Income") %>%
  sf::st_as_sf() %>% 
  ggplot() +
  geom_sf(aes(fill = diff_2021_1819), size = .25) +
  scale_fill_gradient2(low="#e8e3d3", high="#4b2e83", name="") +
...
# ggsave("/Users/alexliu/Desktop/A.3.2.1.ii nonpass diff year nonlow income.png")

```

```{r A.3.2.1.ii match district names with shape file, eval=FALSE, include=FALSE}
...
```{r A.3.2.1.iii The Difference in numerical fail grade from 2018-19 to 2020-21 (low-income and non-low income), message=FALSE, warning=FALSE}
...
wa_numfail_inc <- left_join(df_numfail_inc, wa, by = c("District" = "NAME"), copy=TRUE) 
...
# ggsave("/Users/alexliu/Desktop/A.3.2.1.iii num fail diff year low income.png")
...
# ggsave("/Users/alexliu/Desktop/A.3.2.1.iii num fail diff year nonlow income.png")
```
...
* Gap calculated by %nonpass among low-income minus %nonpass among non-low income in SY2020-21
  + darker purple color indicates more nonpass grades among students of color
...
plot_spatial_white_nonwhite_gap <- ggplot() +
  ...
  theme(panel.background = element_blank(),
        ...
        plot.subtitle = element_text(size = 10), 
        plot.caption = element_text(hjust = 0, size = 7, face = "italic")) +
  labs(title="Gap of All Non-Passing in SY2020-21",
...
```{r A.3.2.2.ii The gap in all non-passing in 2020-21 between students of low-income and non-low income for each district}
...
# ggsave("/Users/alexliu/Desktop/A.3.2.2.ii nonpass gap sy2020-21.png")


```

```{r A.3.2.2.iii The gap in num fail grade in 2020-21 between students of low-income and non-low income for each district}
...
# ggsave("/Users/alexliu/Desktop/A.3.2.2.iii num fail gap sy2020-21.png")
