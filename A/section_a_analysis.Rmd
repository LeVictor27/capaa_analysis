...
* Note 01/11
  + ~~add num pass by income figure to slides~~
  + ~~check graduation data date~~
    + COVID-19 Modified School Year SQSS Data is used for A4
  + ~~chack 9th grade on track suppression~~
    + Suppressed: Cross Group
...
```{r Student Enrollment Count for All Grade Levels, echo=FALSE}
...
#  facet_wrap(~Month, ncol =1)+
  ggtitle("Enrollment Counts in 2021 (All Grade Levels)") +
...
```{r load grade_level enrollment data}
...
long_enroll_race_grades$Month <- factor(long_enroll_race_grades$Month, 
                                        levels = c("February",
                                                   "January",
                                                   "December",
                                                   "November",
...
long_enroll_race_grades %>%
  ...
  ggtitle("Student Enrollment Counts in 2021") +
        labs(x = "Student Groups", 
             y = "Student Counts") +
  scale_y_continuous(labels = comma) +
  scale_fill_manual(values = palette_uw_reverse(5))  +
  theme_uw() +
  coord_flip() +
  guides(fill = guide_legend(reverse = TRUE)) +
...
* do not have non-low income grade level data on ospi website?
...
long_enroll_perc_races$Month <- factor(long_enroll_perc_races$Month,
                                       levels = c("January",
                                                  "December",
                                                  "November",
                                                  "October"))

long_enroll_perc_races %>%
  filter(StudentGroup != "All Students") %>%
  ...
  ggtitle("Enrollment Percentage in 2021 (All Grade Levels)") +
        labs(x = "Student Groups", 
...
```{r Student Enrollment Percentage by income}
enrollment_by_inc <- df_enrollment %>%
  filter(StudentGroupType == "FRL" | StudentGroupType == "All") %>%
  filter(Measure == "2021 Enrolled") %>%
  select(!c(StudentGroupType, Measure)) 
  
long_enroll_by_inc <- melt(enrollment_by_inc, 
                             id.vars = "StudentGroup",
                             measure.vars = month_in_df,
                             variable.name = "Month",
                             value.name="EnrollmentCounts") %>%
...
enrollment_perc_inc <- data.frame(matrix(NA,    
                          nrow = nrow(enrollment_by_inc),
                          ncol = ncol(enrollment_by_inc)))

dimnames(enrollment_perc_inc) <- list(rownames(enrollment_by_inc), 
                                        colnames(enrollment_by_inc))


for (j in 2:ncol(enrollment_by_inc)) {
  for (i in nrow(enrollment_by_inc):1) {
    enrollment_perc_inc[i, j] = percent(enrollment_by_inc[i, j]/enrollment_by_inc[1,j]*100)
    enrollment_perc_inc[i, 1] = enrollment_by_inc[i, 1]
  }
}

long_enroll_perc_inc <- melt(enrollment_perc_inc, 
                             id.vars = "StudentGroup",
                             measure.vars = month_in_df,
                             variable.name = "Month",
                             value.name="EnrollmentPercentage") %>%
...
long_enroll_perc_inc$Month <- factor(long_enroll_perc_inc$Month,
                                       levels = c("January",
                                                  "December",
                                                  "November",
                                                  "October"))

long_enroll_perc_inc %>%
  filter(StudentGroup != "All Students") %>%
  mutate(EnrollmentPercentage = EnrollmentPercentage/100)%>%
  ggplot(aes(fill = Month, 
             x = StudentGroup, 
...
```{r A.1.iv Student Enrollment Percentage for Kindergarteners}
...
```{r A.1.v Student Enrollment Percentage for 6th Graders}
...
```{r A.i.vi Student Enrollment Percentage for 9th Graders}
...
long_enroll_perc$Month <- factor(long_enroll_perc$Month, levels = c("February",
                                                                    "January",
                                                                    "December",
                                                                    "November",
                                                                    "October"))
...
#             hjust = -0.1, vjust = 0.4, colour = "black", size = 1.5) +
  ...
  facet_grid(~factor(GradeLevel, levels=c("Kindergarten", "Sixth Grade", "Ninth Grade"))) +
  ggtitle("Student Enrollment Percentage in 2021") +
        labs(x = "Student Groups", 
             y = "Enrollment Percentage") +
...
```{r Student Enrollment Change for 9th Graders sorted by income, eval=FALSE, include=FALSE}
...
long_change_enroll_race_grades$Month <- factor(long_change_enroll_race_grades$Month, 
                                               levels = c("February",
                                                          "January",
                                                          "December",
                                                          "November",
...
long_change_enroll_race_grades %>%
  ggplot(aes(fill = Month, 
             x = `StudentGroup`, 
  ...
  scale_y_continuous(labels = percent, limits = c(-0.3, 0.1)) +
  geom_hline(yintercept = 0) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
 # geom_text(aes(label = `Change in Enrollment`), position=position_dodge(width=0.8), 
#            hjust = 0.7, vjust = 0.4, colour = "black", size = 1.5) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  theme_uw() +
  coord_flip() +
...
long_change_enroll_race_grades %>%
  ...
  scale_fill_manual(values = palette_uw_reverse(5)) +
 # geom_text(aes(label = `Change in Enrollment`), position=position_dodge(width=0.8), 
 #           hjust = 0, vjust = 0.4, colour = "black", size = 1.5) +
  theme_uw() +
  coord_flip() +
  guides(fill = guide_legend(reverse = TRUE)) +
  theme(axis.text.x  = element_text(angle=45, vjust=0.5)) 
...
long_change_enroll_income_grades$GradeLevel <- factor(long_change_enroll_income_grades$GradeLevel, 
                                                      levels=c("Kindergarten", "Sixth Grade", "Ninth Grade"))
long_change_enroll_income_grades$Month <- factor(long_change_enroll_income_grades$Month,
                                                 levels = c("February",
                                                            "January",
                                                            "December",
                                                            "November",
                                                            "October"))
#horizontal grid
long_change_enroll_income_grades %>%
  ggplot(aes(fill = Month, 
             x = `StudentGroup`, 
             y = `Change in Enrollment`)) + 
  geom_col(position = "dodge", width = 0.7) + 
  ...
  ggtitle("Percentage Change in Student Enrollment",
        ...
        labs(x = "Student Groups", 
             y = "Percentage Change in Enrollment") +
  scale_y_continuous(labels = percent, limits = c(-0.2, 0.1)) +
  geom_hline(yintercept = 0) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
#  geom_text(aes(label = `Change in Enrollment`), position=position_dodge(width=0.8), 
#            hjust = 0.6, vjust = 0.4, colour = "black", size = 1.5) +
  theme_uw() +
  coord_flip() +
  guides(fill = guide_legend(reverse = TRUE)) +
  theme(axis.text.x  = element_text(angle=45, vjust=0.5))
...
long_change_enroll_income_grades %>%
  ...
  scale_fill_manual(values = palette_uw_reverse(5)) +
 # geom_text(aes(label = `Change in Enrollment`), position=position_dodge(width=0.8), 
#            hjust = 0.5, vjust = 0.4, colour = "black", size = 1.5) +
  theme_uw() +
  coord_flip() +
  guides(fill = guide_legend(reverse = TRUE)) +
  theme(axis.text.x  = element_text(angle=45, vjust=0.5))
...
```{r A.2.1.i Student Absence Count sorted by races, fig.height=12, fig.width=10, warning=FALSE, include=FALSE}
...
#  geom_bar(position="dodge", stat="identity") + #to separated bars for months 
  ...
  facet_wrap(~GradeLevel, ncol = 1) +
  ggtitle("Total Absences by Month in 2021") +
        labs(x = "Student Groups", 
             y = "Total Absences") +
...
```{r A.2.1.i Student Absence Count sorted by races unstacked, fig.height=14, fig.width=10, warning=FALSE, include=FALSE}
...
```{r A.2.1.i Student Absence Count sorted by races separate, eval=FALSE, warning=FALSE, include=FALSE}

long_absences_count <- ungroup(long_absences_count)

grade_levels <- unique(long_absences_count$GradeLevel)
plot_absences_count_separate <- list()
long_absences_count$Month <- factor(long_absences_count$Month,
                                    levels = c("February",
                                               "January",
                                               "December",
                                               "November",
                                               "October"))
selected_grade_levels <- c(3, 6, 9)
...
for (i in seq_along(grade_levels)) {
  df <- long_absences_count %>% filter(GradeLevel == grade_levels[i]) 
    ...
    geom_text(aes(label = `Total Absences`), position=position_dodge(width=0.7), 
            hjust = -0.2, vjust = 0.6, colour = "black", size = 1.5) +
    scale_fill_manual(values = palette_uw_reverse(5))  +
    theme_uw() +
    coord_flip() +
    guides(fill = guide_legend(reverse = TRUE)) +
...
```{r *A.2.1.i Student Absence Count sorted by races grade 3 6 9}
...
long_absences_count$Month <- factor(long_absences_count$Month,
                                    levels = c("February",
                                               ...
                                               "November",
                                               "October"))
selected_grade_levels <- c("3rd Grade", "6th Grade", "9th Grade")
...
long_absences_count %>% 
  ...
  ggplot(aes(x = StudentGroup, 
             ...
             order = Month)) + 
  geom_col(position="dodge", width = 0.7) +  
  facet_grid(~factor(GradeLevel, levels=c("3rd Grade", "6th Grade", "9th Grade"))) +
  ggtitle("Total Absences by Month in 2021") +
  labs(x = "Student Groups", y = "Total Absences") +
  geom_hline(yintercept = 0) +
  scale_y_continuous(limits = c(0, 7.5)) +
...
```{r A.2.1.ii Student Absence Count sorted by income data}
...
```{r A.2.1.ii Student Absence Count sorted by income, warning=FALSE, include=FALSE}
...
```{r A.2.1.ii Student Absence Count sorted by income unstacked, fig.height=12, fig.width=10, include=FALSE}
...
```{r A.2.1.ii Student Absence Count sorted by income separate, eval=FALSE, include=FALSE}
long_absences_count_income <- ungroup(long_absences_count_income)

grade_levels <- unique(long_absences_count_income$GradeLevel)
plot_absences_count_inc_separate <- list()
long_absences_count_income$Month <- factor(long_absences_count_income$Month,
                                           levels = c("February",
                                                      "January",
                                                      "December",
                                                      "November",
...
```{r *A.2.1.ii Student Absence Count sorted by income grade 3 6 9}
long_absences_count_income <- ungroup(long_absences_count_income)
...
```{r A.2.2.i Student Absence Change sorted by races, fig.height=14, fig.width=10, warning=FALSE, include=FALSE}
long_absences_change$Month <- factor(long_absences_change$Month,
                                     levels = c("February",
                                                "January",
                                                "December",
                                                "November",
...
#  geom_bar(stat="identity", width = 0.4) +
  ...
  ggtitle("Difference in Total Absences",
          subtitle = "between 2019 and 2021") +
        labs(x = "Student Groups", 
...
```{r A.2.2.i Student Absence Change sorted by races separate, eval=FALSE, include=FALSE}
...
for (i in seq_along(grade_levels)) {
  df <- long_absences_change %>% filter(GradeLevel == grade_levels[i]) 
    p<- ggplot(df, aes(x = StudentGroup, 
      ...
      scale_y_continuous(limits = c(-1, 3.5)) +
      geom_text(aes(label = `% Change in Absences`), position=position_dodge(width=0.7), 
            hjust = -0.1, vjust = 0.4, colour = "black", size = 1.5) +
      scale_fill_manual(values = palette_uw_reverse(5))  +
      theme_uw() +
      coord_flip() +
...
```{r A.2.2.ii Student Absence Change sorted by income, fig.height=12, fig.width=10, warning=FALSE, include=FALSE, paged.print=TRUE}
long_absences_change_income$Month <- factor(long_absences_change_income$Month,
                                            levels = c("February",
                                                       "January",
                                                       "December",
                                                       "November",
...
long_absences_change_income %>%
  ...
  ggtitle("Difference in Total Absences",
        ...
        labs(x = "Student Groups", 
             y = "Difference in Absences") +
  geom_text(aes(label = `% Change in Absences`), position = position_dodge(0.7),
            hjust = -0.1, vjust = 0.4, colour = "black", size = 1.5) +
...
```{r A.2.2.ii Student Absence Change sorted by income separate, eval=FALSE, include=FALSE}
...
```{r A.2.2.iii Student Absence PERCENTAGE Change sorted by races}
...
```{r *A.2.2.iii Student Absence PERCENTAGE Change sorted by races grade 3 6 9}

selected_grade_levels <- c("3rd Grade", "6th Grade", "9th Grade")

long_absences_perc_change %>%
  filter(GradeLevel %in% selected_grade_levels) %>%
  ggplot(aes(x = StudentGroup, 
             y = `% Change in Absences`,
             fill = Month,
  ...
  labs(x = "Student Groups", y = "% Change in Absences") +
  scale_y_continuous(labels = percent, limits = c(-1, 1.8)) +
  geom_hline(yintercept = 0) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
 # geom_text(aes(label = `% Change in Absences`), position=position_dodge(width=0.9), 
...
```{r A.2.2.iii Student Absence PERCENTAGE Change sorted by income}
...
```{r *A.2.2.iii Student Absence PERCENTAGE Change sorted by income grade 3 6 9}
long_absences_perc_change_income %>%
  filter(GradeLevel %in% selected_grade_levels) %>%
  ggplot(aes(x = StudentGroup, 
             y = `% Change in Absences`,
             fill = Month, 
...
course_by_race$`Grade Level` <- 
  factor(course_by_race$`Grade Level`, levels = c("All Grades", "12", "11", "10", "9"))
...
course_by_race %>%
  ...
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
 # geom_text(aes(label = `All Non-Passing Grades`), position = position_dodge(0.7),
#            hjust = -0.1, vjust = 0.4, colour = "black", size = 1.5) +
  theme_uw() +
  coord_flip() +
  guides(fill = guide_legend(reverse = TRUE)) 
...
```{r A.3.1.ii Statewide breakdown by income, message=FALSE, warning=FALSE}
...
course_by_income$`Grade Level` <- 
  factor(course_by_income$`Grade Level`, levels = c("All Grades", "12", "11", "10", "9"))
...
```{r A.3.1.iii Statewide breakdown by groups all passing grade, message=FALSE, warning=FALSE, include=FALSE}

course_by_race_pass <- df_course_outcomes_by_outcome %>%
  filter(`Organization Name` == "Washington State" & 
           !is.na(`All Passing Grades`)) %>%
  select_if(~!all(is.na(.))) %>%
  mutate(`All Passing Grades` =  percent(`All Passing Grades`))

#state_ave_20_21 <- percent('11.0%')
  
course_by_race_pass$`Grade Level` <- 
  factor(course_by_race_pass$`Grade Level`, levels = c("All Grades", "12", "11", "10", "9"))
course_by_race_pass$`School Year` <- 
  factor(course_by_race_pass$`School Year`, levels = c("2020-21", "2019-20", "2018-19"))

course_by_race_pass %>%
  filter(`School Year` == "2020-21") %>%
  ggplot(aes(fill= `Grade Level`,
             x= `Student Group`, 
             y=`All Passing Grades`)) + 
  geom_col(position="dodge", stat="identity", width = 0.7) + 
#  geom_hline(yintercept = 0.11, linetype = "dashed") +
#  geom_text(aes(0, 0.11, label = "state average = 11%"), size = 3, vjust = -0.5) +
  ggtitle("Percentage of All Courses Taken Resulted in Passing Grade", 
          subtitle = "2021") +
  labs(x = "Student Groups", y = "% All Passing Grades") +
#  scale_y_continuous(labels = percent, limits = c(0, 0.3)) +
  scale_y_continuous(labels = percent) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
...
```{r *A.3.1.iv Statewide breakdown by groups all passing grade}

course_by_race_pass_2021 <- course_by_race_pass %>%
  filter(`School Year` == "2020-21") 

for (i in 1:length(course_by_race_pass_2021$`All Passing Grades`)) {
  if (course_by_race_pass_2021$`Student Group`[i] %in% c("Asian", "White")) {
     course_by_race_pass_2021$`All Passing Grades`[i] <- 1
  }
}


course_by_race_pass_2021 %>%  
  ggplot(aes(fill= `Grade Level`,
             x= `Student Group`, 
             y=`All Passing Grades`)) + 
  geom_col(position="dodge", stat="identity", width = 0.7) + 
...
#  geom_text(aes(0, 0.11, label = "state average = 11%"), size = 3, vjust = -0.5) +
  ...
  labs(x = "Student Groups", 
       y = "% All Passing Grades",
       caption = "Note: missing values are suppressed") +
#  scale_y_continuous(labels = percent, limits = c(0, 0.3)) +
  scale_y_continuous(labels = percent) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
...
```{r **A.3.1.v Statewide breakdown by race num pass}

course_by_race_num_pass <- df_course_outcomes_by_outcome %>%
  filter(`Organization Name` == "Washington State" & 
           !is.na(`Numerical Passing Grades`))

## treat suppressed>99% as 100%
for (i in 1:length(course_by_race_num_pass$`Numerical Passing Grades`)) {
  if (course_by_race_num_pass$`Numerical Passing Grades`[i] == "Suppressed: >99%") {
     course_by_race_num_pass$`Numerical Passing Grades`[i] <- 1
  }
}

course_by_race_num_pass <- course_by_race_num_pass %>%
  select_if(~!all(is.na(.))) %>%
  mutate(`Numerical Passing Grades` =  percent(`Numerical Passing Grades`)) 

#state_ave_20_21 <- percent('11.0%')
  
course_by_race_num_pass$`Grade Level` <- 
  factor(course_by_race_num_pass$`Grade Level`, levels = c("All Grades", "12", "11", "10", "9"))
course_by_race_num_pass$`School Year` <- 
  factor(course_by_race_num_pass$`School Year`, levels = c("2020-21", "2019-20", "2018-19"))

course_by_race_num_pass %>%
  filter(`School Year` == "2020-21") %>%
  ggplot(aes(fill= `Grade Level`,
             x= `Student Group`, 
             y=`Numerical Passing Grades`)) + 
  geom_col(position="dodge", stat="identity", width = 0.7) + 
  geom_hline(yintercept = 0.804, linetype = "dashed") +
  geom_text(aes(0, 0.11, label = "state average = 80.4%"), 
            size = 3, vjust = -0.5, hjust = -2) +
  ggtitle("Percentage of Courses Taken Resulted in Numerical Passing Grades", 
          subtitle = "2021") +
  labs(x = "Student Groups", 
       y = "% Numerical Passing Grades") +
#  scale_y_continuous(labels = percent, limits = c(0, 0.3)) +
...
course_by_race_num_pass %>%
  ...
  ggplot(aes(fill= `Grade Level`,
             ...
             y=`Numerical Passing Grades`)) + 
  geom_col(position="dodge", stat="identity", width = 0.7) + 
  geom_hline(yintercept = .995, linetype = "dashed") +
  geom_text(aes(0, 0.11, label = "state average > 99%"), 
            size = 3, vjust = -0.5, hjust = -2.7) +
  ggtitle("Percentage of Courses Taken Resulted in Numerical Passing Grades", 
          subtitle = "2020") +
  labs(x = "Student Groups", 
       y = "% Numerical Passing Grades") +
...
```{r check suppression}
test <- df_course_outcomes_by_outcome %>%
  filter(`Organization Name` == "Washington State") %>%
  select(c("Numerical Passing Grades", "Student Group", 
           "Grade Level", "School Year")) %>%
  drop_na()
```

```{r **A.3.1.iv Statewide breakdown by income num pass, message=FALSE, warning=FALSE}
id_num_pass <- drive_get(as_id("https://drive.google.com/file/d/1_Y7mP_Lk8IpD2uj8tvvwldRVfK-HRYh_/view?usp=sharing"))

course_by_inc_num_pass <- id_num_pass %>%
  drive_read_string() %>%
  read_csv(col_names=TRUE) %>%
  mutate(`Numerical Passing Grades` = `All Passing Grades`)

## treat suppressed>99% as 100%
for (i in 1:length(course_by_inc_num_pass$`Numerical Passing Grades`)) {
  if (course_by_inc_num_pass$`Numerical Passing Grades`[i] == "Suppressed:>99%") {
     course_by_inc_num_pass$`Numerical Passing Grades`[i] <- 1
  }
}

course_by_inc_num_pass <- course_by_inc_num_pass %>%
  mutate(`Numerical Passing Grades` = percent(`Numerical Passing Grades`))
  
course_by_inc_num_pass$`Grade Level` <- 
  factor(course_by_inc_num_pass$`Grade Level`, levels = c("All Grades", "12", "11", "10", "9"))
course_by_inc_num_pass$`School Year` <- 
  factor(course_by_inc_num_pass$`School Year`, levels = c("2020-21", "2019-20", "2018-19"))

course_by_inc_num_pass %>%
  filter(`School Year` == "2020-21") %>%
  ggplot(aes(fill= `Grade Level`,
             x= `Student Group`, 
             y=`Numerical Passing Grades`)) + 
  ...
  geom_hline(yintercept = 0.804, linetype = "dashed") +
  geom_text(aes(0, 0.11, label = "state average = 80.4%"), 
            size = 3, vjust = -0.5, hjust = -2.3) +
  ggtitle("Percentage of Courses Taken Resulted in Numerical Passing Grade", 
          subtitle = "2021") +
  labs(x = "Student Groups", y = "% Numerical Passing Grades") +
  scale_y_continuous(labels = percent) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
#  geom_text(aes(label = `All Non-Passing Grades`), position = position_dodge(0.7),
#            hjust = -0.1, vjust = 0.4, colour = "black", size = 1.5) +
...
**% Change is calculated by `All Non-Passing Grades`[`School Year` == "2020-21"] - `All Non-Passing Grades`[`School Year` == "2018-19"]/`All Non-Passing Grades`[`School Year` == "2018-19"]**

## A.3.1. Statewide change (SY 2020-21 - SY 2018-19)/SY 2018-19
...
course_by_race <- course_by_race %>%
  ...
  mutate(Change = (`All Non-Passing Grades`[`School Year` == "2020-21"] - `All Non-Passing Grades`[`School Year` == "2018-19"])/`All Non-Passing Grades`[`School Year` == "2018-19"])

course_by_race$`Grade Level` <- factor(course_by_race$`Grade Level`,
                                       levels = c("All Grades", "12", "11", "10", "9"))
...
course_by_race %>%
  ...
  geom_col(position="dodge", width = 0.7) +
  geom_hline(yintercept = 0) +
  ggtitle("Percentage Change in All Courses Taken Resulted in Non-Passing Grade", 
          subtitle = "2019 vs. 2021") +
  labs(x = "Student Groups", 
       y = "% Change",
       caption = "A positive value means that the percentage of all courses taken resulted in non-passing grade \nhas increased between 2019 and 2021 for the given student group") +
  scale_y_continuous(labels = percent) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
 # geom_text(aes(label = `Change`), position = position_dodge(0.7),
#            hjust = -0.1, vjust = 0.4, colour = "black", size = 1.5) +
  theme_uw() +
  coord_flip() +
  guides(fill = guide_legend(reverse = TRUE)) 
...
course_by_income <- course_by_income %>%
  group_by(`Student Group`, `Grade Level`) %>%
  mutate(Change = (`All Non-Passing Grades`[`School Year` == "2020-21"] - `All Non-Passing Grades`[`School Year` == "2018-19"])/`All Non-Passing Grades`[`School Year` == "2018-19"])
course_by_income$`Grade Level` <- factor(course_by_income$`Grade Level`,
                                         levels = c("All Grades", "12", "11", "10", "9"))
course_by_income %>%
  filter(`School Year` == "2020-21") %>%
  ggplot(aes(fill= `Grade Level`,
  ...
  labs(x = "Student Groups", 
       y = "% Change",
       caption = "A positive value means that the percent of all courses taken resulted in non-passing grade \nhas increased between 2019 and 2021 for the given student group") +
  scale_y_continuous(labels = percent) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
 # geom_text(aes(label = `Change`), position = position_dodge(0.7),
#            hjust = -0.1, vjust = 0.4, colour = "black", size = 1.5) +
...
```{r *A.3.1.2.iii Statewide breakdown by race all passing CHANGE, message=FALSE, warning=FALSE}

course_by_race_pass <- course_by_race_pass %>%
  group_by(`Student Group`, `Grade Level`) %>%
  mutate(Change = (`All Passing Grades`[`School Year` == "2020-21"] - `All Passing Grades`[`School Year` == "2018-19"])/`All Passing Grades`[`School Year` == "2018-19"])

course_by_race_pass$`Grade Level` <- factor(course_by_race_pass$`Grade Level`,
                                       levels = c("All Grades", "12", "11", "10", "9"))
...
course_by_race_pass %>%
  ...
  geom_col(position="dodge", width = 0.7) +
  geom_hline(yintercept = 0) +
  ggtitle("Percentage Change in All Courses Taken Resulted in Passing Grade", 
          subtitle = "2019 vs. 2021") +
  labs(x = "Student Groups", 
       y = "% Change",
       caption = "A positive value means that the percentage of all courses taken resulted in passing grade \nhas increased between 2019 and 2021 for the given student group") +
  scale_y_continuous(labels = percent) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
...
```{r **A.3.1.2.iv Statewide breakdown by race num pass CHANGE, message=FALSE, warning=FALSE}

course_by_race_num_pass <- course_by_race_num_pass %>%
  group_by(`Student Group`, `Grade Level`) %>%
  mutate(Change = (`Numerical Passing Grades`[`School Year` == "2020-21"] - `Numerical Passing Grades`[`School Year` == "2018-19"])/`Numerical Passing Grades`[`School Year` == "2018-19"])

course_by_race_num_pass$`Grade Level` <- factor(course_by_race_num_pass$`Grade Level`,
                                       levels = c("All Grades", "12", "11", "10", "9"))
...
course_by_race_num_pass %>%
  ...
  geom_col(position="dodge", width = 0.7) +
  geom_hline(yintercept = 0) +
  ggtitle("Percent Change in Courses Taken Resulted in Numerical Passing Grades", 
          subtitle = "2019 vs. 2021") +
  labs(x = "Student Groups", 
       y = "% Change",
       caption = "Note: A negative value means that the percentage of all courses taken resulted in numerical passing \ngrades has decreased in 2021 compared to 2019 for the given student group") +
  scale_y_continuous(labels = percent) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  scale_fill_manual(values = palette_uw_reverse(5)) +
...
```{r **A.3.1.2.iv Statewide breakdown by inc num pass CHANGE, message=FALSE, warning=FALSE}

course_by_inc_num_pass <- course_by_inc_num_pass %>%
  group_by(`Student Group`, `Grade Level`) %>%
  mutate(Change = (`Numerical Passing Grades`[`School Year` == "2020-21"] - `Numerical Passing Grades`[`School Year` == "2018-19"])/`Numerical Passing Grades`[`School Year` == "2018-19"])

course_by_inc_num_pass$`Grade Level` <- factor(course_by_inc_num_pass$`Grade Level`,
                                       levels = c("All Grades", "12", "11", "10", "9"))
...
> A.3.2.1 Geographical differences across districts in all non-passing grade and numerical failing grade  (SY2020-21 - SY2018-19)/SY2018-19
...
* Difference calculated by (%nonpass (or %numerical failing) in SY2020-21 minus %nonpass (or %numerical failing) in SY 2018-19) divided by %nonpass (or %numerical failing) in SY 2018-19
...
df_nonpass_race <- merge(x = df_nonpass_race_2021, y = df_nonpass_race_1819, 
  ...
  mutate(Change = (`SY2020-21` - `SY2018-19`)/`SY2018-19`) %>%
  filter(!is.na(Change)) %>%
  #filter(Change >= -1) %>%
  mutate(Race = NA) 
...
wa_nonpass_white %>% 
  ...
  theme_uw_map() +
  labs(title="Percent Change in All Courses Taken Resulted in No Credit",
       subtitle = "for white students (2019 vs. 2021)",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in non-passing grade \nhas increased between 2019 and 2021 for the given student group")
...
wa_nonpass_nonwhite %>% 
  ...
  theme_uw_map() +
  labs(title="Percent Change in All Courses Taken Resulted in No Credit",
       subtitle = "for students of color (2019 vs. 2021)",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in non-passing grade \nhas increased between 2019 and 2021 for the given student group")
...
wa_numfail_white %>% 
  ...
  scale_fill_gradient2(low="#e8e3d3", high="#4b2e83", name="") +
  theme_uw_map() +
  labs(title="Percent Change in All Courses Taken Resulted in Numerical Fail Grade",
       subtitle = "for white students (2019 vs. 2021)",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in numerical failing \ngrade has increased between 2019 and 2021 for the given student group")
...
```{r A.3.2.1.iii The Difference in all non-passing grade from 2018-19 to 2020-21 (low-income and non-low income), echo=FALSE, message=FALSE}
...
df_nonpass_inc <- df_nonpass_inc_raw %>%
  ...
  mutate(diff_2021_1819 = (`2020-21` - `2018-19`)/`2018-19`) %>%
  arrange(District) %>%
  group_by(District) %>% 
...
wa_nonpass_low %>% 
  ...
  theme_uw_map() +
  labs(title="Percent Change in All Courses Taken Resulted in No Credit",
       subtitle = "for low-income student group (2019 vs. 2021)",
...
wa_nonpass_nonlow %>% 
  ...
  theme_uw_map() +
  labs(title="Percent Change in All Courses Taken Resulted in No Credit",
       subtitle = "for non-low income student group (2019 vs. 2021)",
       caption = "Note: grey areas indicate districts with unavaible data \nA positive value means that the percent of all courses taken resulted in non-passing grade \nhas increased between 2019 and 2021 for the given student group")
...
```{r A.3.2.1.iv The Difference in numerical fail grade from 2018-19 to 2020-21 (low-income and non-low income), echo=FALSE, message=FALSE, warning=FALSE}
...
```{r *A.3.2.1.v The Difference in all passing grade from 2018-19 to 2020-21 (white and nonwhite), echo=FALSE, message=FALSE, warning=FALSE}

df_pass_race_raw <- df_course_outcomes_by_outcome %>%
  select(c(`Organization Name`, `School Year`, 
           `Student Group`, `Grade Level`, 
           `All Passing Grades`)) %>%
  filter(!is.na(`All Passing Grades`) &
           `School Year` != "2019-20") %>% 
  mutate(`All Passing Grades` =  percent(`All Passing Grades`))

#cannot use dcast due to uneven length for each year:
#split the df by school year; rename `All Non-Passing Grades` column by SY; 
df_pass_race_1819 <- filter(df_pass_race_raw, `School Year` != "2020-21") 
colnames(df_pass_race_1819)[5] <- "SY2018-19"
  
df_pass_race_2021 <- filter(df_pass_race_raw, `School Year` == "2020-21")
colnames(df_pass_race_2021)[5] <- "SY2020-21"

#merge yearly dfs and calculate the change
df_pass_race <- merge(x = df_pass_race_2021, y = df_pass_race_1819, 
                         by = c("Organization Name", "Student Group", "Grade Level"), 
                         all = TRUE) %>%
  select(!c("School Year.x", "School Year.y")) %>%
  mutate(Change = (`SY2020-21` - `SY2018-19`)/`SY2018-19`) %>%
...
for (i in 1:nrow(df_pass_race)) {
  if (df_pass_race[i, "Student Group"] == "White") {
    df_pass_race[i, "Race"] <- "White"
  } else if (df_pass_race[i, "Student Group"] == "Asian") {
    df_pass_race[i, "Race"] <- "Asian"
  } else {
    df_pass_race[i, "Race"] <- "Non-white"
  }
}

df_pass_race_grouped <- aggregate(df_pass_race$Change, 
                           list(df_pass_race$`Organization Name`, 
                                df_pass_race$`Grade Level`,
                                df_pass_race$Race), 
                           FUN=mean)

names(df_pass_race_grouped) <- c("Districts", "Grade Level", "Race", "Change")
df_pass_race_grouped <- df_pass_race_grouped %>%
  arrange(`Grade Level`, Districts, Race) #%>%
  #filter(`Grade Level` == "All Grades")
...
df_pass_white <- filter(df_pass_race_grouped, Race == "White") 
wa_pass_white <- full_join(wa, df_pass_white, by = c("NAME" = "Districts"), copy=TRUE)

df_pass_nonwhite <- filter(df_pass_race_grouped, Race == "Non-white") 
wa_pass_nonwhite <- full_join(wa, df_pass_nonwhite, by = c("NAME" = "Districts"), copy=TRUE)
...
wa_pass_white %>% 
  ...
  scale_fill_gradient2(low="#e8e3d3", high="#4b2e83", name="") +
  theme_uw_map() +
  labs(title="Percent Change in All Courses Taken Resulted in Credits",
       subtitle = "for white students (2019 vs. 2021)",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in passing grades \nhas increased between 2019 and 2021 for the given student group")
...
wa_pass_nonwhite %>% 
  ...
  labs(title="Percent Change in All Courses Taken Resulted in Credits",
       subtitle = "for students of color (2019 vs. 2021)",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in passing grade \nhas increased between 2019 and 2021 for the given student group")
...
test <- df_course_outcomes_by_outcome %>%
  ...
  drop_na() %>%
  filter(`Grade Level` == "All Grades") %>% 
  filter(`School Year` %in% c ("2018-19", "2020-21"))


  
```

```{r omit A.3.2.1.vi The Difference in num pass from 2018-19 to 2020-21 white, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}

df_num_pass_race_raw <- df_course_outcomes_by_outcome %>%
  mutate(`Numerical Passing Grades` =  percent(`Numerical Passing Grades`)) %>%
  select(c(`Organization Name`, `School Year`, 
           `Student Group`, `Grade Level`, 
           `Numerical Passing Grades`)) %>%
  filter(!is.na(`Numerical Passing Grades`) &
           `School Year` != "2019-20") 

#cannot use dcast due to uneven length for each year:
#split the df by school year; rename `All Non-Passing Grades` column by SY; 
df_num_pass_race_1819 <- filter(df_num_pass_race_raw, `School Year` != "2020-21") 
colnames(df_num_pass_race_1819)[5] <- "SY2018-19"
  
df_num_pass_race_2021 <- filter(df_num_pass_race_raw, `School Year` == "2020-21")
colnames(df_num_pass_race_2021)[5] <- "SY2020-21"

#merge yearly dfs and calculate the change
df_num_pass_race <- merge(x = df_num_pass_race_2021, y = df_num_pass_race_1819, 
                         by = c("Organization Name", "Student Group", "Grade Level"), 
                         all = TRUE) %>%
  select(!c("School Year.x", "School Year.y")) %>%
  mutate(Change = (`SY2020-21` - `SY2018-19`)/`SY2018-19`) %>%
...
for (i in 1:nrow(df_num_pass_race)) {
  if (df_num_pass_race[i, "Student Group"] == "White") {
    df_num_pass_race[i, "Race"] <- "White"
  } else if (df_num_pass_race[i, "Student Group"] == "Asian") {
    df_num_pass_race[i, "Race"] <- "Asian"
  } else {
    df_num_pass_race[i, "Race"] <- "Non-white"
  }
}

df_num_pass_race_grouped <- aggregate(df_num_pass_race$Change, 
                           list(df_num_pass_race$`Organization Name`, 
                                df_num_pass_race$`Grade Level`,
                                df_num_pass_race$Race), 
                           FUN=mean)

names(df_num_pass_race_grouped) <- c("Districts", "Grade Level", "Race", "Change")
df_num_pass_race_grouped <- df_num_pass_race_grouped %>%
  arrange(`Grade Level`, Districts, Race) %>%
  filter(`Grade Level` == "All Grades")
...
df_num_pass_white <- filter(df_num_pass_race_grouped, Race == "White") 
wa_num_pass_white <- full_join(wa, df_num_pass_white, by = c("NAME" = "Districts"), copy=TRUE)

df_num_pass_nonwhite <- filter(df_num_pass_race_grouped, Race == "Non-white") 
wa_num_pass_nonwhite <- full_join(wa, df_num_pass_nonwhite, by = c("NAME" = "Districts"), copy=TRUE)
...
wa_num_pass_white %>% 
  ...
  scale_fill_gradient2(low="#e8e3d3", high="#4b2e83", name="") +
  theme_uw_map() +
  labs(title="Percent Change in Courses Taken Resulted in Numerical Passing Grades",
       subtitle = "for white students (2019 vs. 2021)",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in numerical passing grades \nhas increased in 2021 compared to 2019 for the given student group")

#ggsave("/Users/alexliu/Desktop/A.3.2.1.i nonpass diff year white.png")

wa_num_pass_nonwhite %>% 
  sf::st_as_sf() %>% 
  ggplot() +
  geom_sf(aes(fill = Change), size = .25) +
  scale_fill_gradient2(low="#e8e3d3", high="#4b2e83", name="") +
...
```{r **A.3.2.1.vi The Difference in num pass from 2018-19 to 2020-21 white}
num_pass_white <- df_course_outcomes_by_outcome %>%
  filter(`Grade Level` == "All Grades") %>%
  select(c("Organization Name", "School Year", "Student Group", 
           "Grade Level", "Numerical Passing Grades")) %>%
  drop_na() %>%
  mutate(`Numerical Passing Grades` = percent(`Numerical Passing Grades`)) %>%
  drop_na() %>%
  filter(`Student Group` == "White") %>%
  filter(`School Year` != "2019-20") %>%
  spread(`School Year`, `Numerical Passing Grades`) %>%
  mutate(change = (`2020-21` - `2018-19`)/`2018-19`) 

# Shape file
wa <- lea_get("wa")%>% 
  sf::st_as_sf() 

wa_num_pass_white <- full_join(wa, num_pass_white, by = c("NAME" = "Organization Name"), copy=TRUE)
...
wa_nonpass_race_gap %>% 
  ...
  labs(title="Gap in All Courses Taken Resulted in No Credit",
       subtitle = "students of color vs. white students in 2021",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in no credit among students \nof color is greater than the percent for white students")
...
wa_numfail_race_gap %>% 
  ...
  labs(title="Gap in All Courses Taken Resulted in Numerical Failing Grade",
       subtitle = "students of color vs. white students in 2021",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in numerical failing grade \namong students of color is greater than the percent for white students")

#ggsave("/Users/alexliu/Desktop/A.3.2.2.ii num fail gap nonwhite white.png")

```

```{r *A.3.2.2.iii The gap in all passing in 2020-21 between students of color and white for each district, echo=FALSE, message=FALSE, warning=FALSE}

df_pass_race_gap_raw <- df_course_outcomes_by_outcome %>%
  select(c(`Organization Name`, `School Year`, 
           `Student Group`, `Grade Level`, 
           `All Passing Grades`)) %>%
  mutate(`All Passing Grades` =  percent(`All Passing Grades`)) %>%
...
for (i in 1:nrow(df_pass_race_gap_raw)) {
  if (df_pass_race_gap_raw[i, "Student Group"] == "White") {
    df_pass_race_gap_raw[i, "Race"] <- "White"
  } else {
    df_pass_race_gap_raw[i, "Race"] <- "Non-white"
  }
}
df_pass_race_gap_grouped <- aggregate(df_pass_race_gap_raw$`All Passing Grades`, 
                           list(df_pass_race_gap_raw$`Organization Name`, 
                                df_pass_race_gap_raw$`Grade Level`,
                                df_pass_race_gap_raw$Race), 
                           FUN=mean)
names(df_pass_race_gap_grouped) <- c("Districts", 
                                     "Grade Level", 
                                     "Race", 
                                     "Ave All Passing Grades")

df_pass_gap_white <- filter(df_pass_race_gap_grouped, Race == "White") 
colnames(df_pass_gap_white)[4] <- "Ave All Passing Grades White"
df_pass_gap_nonwhite <- filter(df_nonpass_race_gap_grouped, Race == "Non-white") 
colnames(df_pass_gap_nonwhite)[4] <- "Ave All Passing Grades Non_White"

df_pass_race_gap <- merge(x = df_pass_gap_white, 
                             y = df_pass_gap_nonwhite, 
                             by = c("Districts", "Grade Level"), 
                             all = TRUE) %>%
  select(!c("Race.x", "Race.y")) %>%
  mutate(Change = `Ave All Passing Grades Non_White` - `Ave All Passing Grades White`) %>%
  arrange(`Grade Level`, Districts) #%>%
  #filter(`Grade Level` == "All Grades") 
...
wa_pass_race_gap <- full_join(wa, df_pass_race_gap, by = c("NAME" = "Districts"), copy=TRUE)
...
wa_pass_race_gap %>% 
  ...
  scale_fill_gradient2(low="#e8e3d3", high="#4b2e83", name="") +
  theme_uw_map() +
  labs(title="Gap in All Courses Taken Resulted in Credits",
       subtitle = "students of color vs. white students in 2021",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in credits among students \nof color is greater than the percent for white students")
...
```{r *A.3.2.2.iii The gap in all non-passing in 2020-21 between students of low-income and non-low income for each district, echo=FALSE}
...
wa_nonpass_gap %>% 
  ...
  theme_uw_map() +
  labs(title="Gap in All Courses Taken Resulted in No Credit",
       subtitle = "low-income student group vs. non-low income student group in 2021",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in no credit among \nlow-income students is greater than the percent for non-low income students")
...
```{r *A.3.2.2.iv The gap in num fail grade in 2020-21 between students of low-income and non-low income for each district, echo=FALSE}
...
wa_numfail_gap %>% 
  ...
  labs(title="Gap in All Courses Taken Resulted in Numerical Failing Grade",
       subtitle = "low-income student group vs. non-low income student group in 2021",
       caption = "Note: grey areas indicate districts with unavaible or suppressed data \nA positive value means that the percent of all courses taken resulted in numerical failing grade among \nlow-income students is greater than the percent for non-low income students")
...
df_state_ontrack_race %>%
  ...
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  scale_fill_manual(values = palette_uw_reverse(3)) +
...
df_state_ontrack_race %>%
  ...
  scale_y_continuous(labels = percent, limits = c(0, 1)) +
  scale_fill_manual(values = palette_uw_reverse(8)) +
  geom_text(aes(label = `% On Track` ), position = position_dodge(0.75),
            hjust = -0.1, vjust = 0.5, colour = "black", size = 2) +
  theme_uw() +
  coord_flip() +
...
```{alternative plot income, eval=FALSE, include=FALSE}
df_state_ontrack_inc %>%
  ggplot(aes(fill= `StudentGroup`,
             x= `SchoolYear`, 
             y= `% On Track`)) + 
...
state_ontrack_race_change <- df_state_ontrack_race %>%
    ...
    mutate(Change = (`% On Track`[SchoolYear == 2020] - `% On Track`[SchoolYear == 2018])/`% On Track`[SchoolYear == 2018])

state_ontrack_race_change %>%
  filter(SchoolYear == 2020) %>%
  ggplot(aes(x= `StudentGroup`, 
             y= `Change`)) + 
  geom_col(position="dodge", width = 0.7, fill = "#4b2e83") +
  ggtitle("Percentage Change in 9th Grade Students on Track",
          subtitle = "2018 vs. 2020") +
  labs(x = "Student Groups",
       y = "% Change",
       caption = "Note: missing values are suppressed \nA negative value means that the percentage of 9th grade students on track has decreased \nbetween 2018 and 2020 for the given student group") +
  scale_y_continuous(labels = percent) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
...
state_ontrack_inc_change %>%
  ...
  labs(x = "Student Groups",
       y = "% Change",
       caption = "Note:  negative value means that the percentage of 9th grade students on track \nhas decreased between 2018 and 2020 for the given student group") +
...
```{r alternative plot reg att, eval=FALSE, include=FALSE}
df_state_regattend_race %>%
  filter(`Grade Level` == "All Grades") %>%
  ggplot(aes(fill= `StudentGroup`,
             x= `SchoolYear`, 
...
```{r alternative plot reg attd income, eval=FALSE, include=FALSE}
df_state_regattend_inc %>%
  filter(`Grade Level` == "All Grades") %>%
  ggplot(aes(fill= `StudentGroup`,
             x= `SchoolYear`, 
...
percentage change (%2021 - %1819)/%1819
...
state_regatt_race_change <- df_state_regattend_race %>%
  ...
  filter(`Grade Level` == "All Grades") %>%
  mutate(Change = (`% Regular Attend`[SchoolYear == 2020] - `% Regular Attend`[SchoolYear == 2018])/`% Regular Attend`[SchoolYear == 2018])

state_regatt_race_change %>%
  ggplot(aes(x= `StudentGroup`, 
             y= `Change`)) + 
  geom_col(position="dodge", width = 0.7, fill = "#4b2e83") +
  ggtitle("Percentage Change of Regular Attendance",
          subtitle = "for all grades (2018 vs. 2020)") +
  labs(x = "Student Groups",
       y = "% Change",
       caption = "Note: a negative value means that the percentage of 9th grade students on \ntrack has decreased between 2018 and 2020 for the given student group") +
  scale_y_continuous(labels = percent, limits = c(-0.08, 0.01)) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
#  geom_text(aes(label = `Change` ), position = position_dodge(0.75),
#            hjust = 1, vjust = 0.5, colour = "black", size = 2) +
...
state_regatt_inc_change %>%
  ggplot(aes(x= `StudentGroup`, 
             y= `Change`)) + 
  geom_col(position="dodge", width = 0.7, fill = "#4b2e83") +
  ggtitle("Percentage Change in Regular Attendance",
          subtitle = "for all grades (2018 vs. 2020)") +
  labs(x = "Student Groups",
       y = "% Change",
       caption = "Note: a negative value means that the percentage of 9th grade students on track \nhas decreased between 2018 and 2020 for the given student group") +
  scale_y_continuous(labels = percent, limits = c(-0.025, 0.0)) +
#  geom_text(aes(label = `Change` ), position = position_dodge(0.75),
#            hjust = 1, vjust = 0.5, colour = "black", size = 2) +
  theme_uw() +
  coord_flip() +
...
df_state_dual_race %>%
  ...
  scale_y_continuous(labels = percent, limits = c(0, 1)) +
  scale_fill_manual(values = palette_uw_reverse(3)) +
 # geom_text(aes(label = PercentTakingCollegeInTheHighSchool), position = position_dodge(0.75),
#            hjust = -0.1, vjust = 0.5, colour = "black", size = 2) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  theme_uw() +
...
df_state_dual_race %>%
  ...
  scale_y_continuous(labels = percent, limits = c(0, 1)) +
  scale_fill_manual(values = palette_uw_reverse(3)) +
#  geom_text(aes(label = PercentTakingCTETechPrep), position = position_dodge(0.75),
#            hjust = -0.1, vjust = 0.5, colour = "black", size = 2) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  theme_uw() +
...
Graduation pathways (plotted) | Notation
----------------------------- | ----------------------------------
"ELA and Math Both"           | met both an ELA and Math Pathway
"CTE"                         | met CTE Pathway
"ASVAB"                       | met ASVAB Pathway
"ELA Assessment"              | completed an ELA Assessment/Test
"ELA Course"                  | completed an ELA Course
"Math Assessment"             | completed an Math Assessment/Test
"Math Course"                 | completed a Math Course

Other paythways (not plotted):

"ELA State Test"    "ELA ACT"           "ELA SAT"           "ELA AP Test"      "ELA IB Test"       
"ELA AP Course"     "ELA IB Course"     "ELA Bridge"        "ELA Cambridge"    "ELA Dual Credit"   
"Math State Test"   "Math ACT"          "Math SAT"          "Math AP Test"     "Math IB Test"      
"Math AP Course"    "Math IB Course"    "Math Bridge"       "Math Cambridge"   "Math Dual Credit"

```{r *A.5.2.i graduation pathway, aggregate state total, by race}

df_state_pathway <- df_graduation_pathways %>%
  filter(`School Name` == "District Total") %>%
  drop_na() 

for (i in 10:ncol(df_state_pathway)) {
  df_state_pathway[i] <- round(df_state_pathway[i] * df_state_pathway[9], 0)
}

state_agg_path <- aggregate(df_state_pathway[,9:ncol(df_state_pathway)], 
                            df_state_pathway[7], FUN = sum )

for (i in 3:ncol(state_agg_path)) {
  state_agg_path[i] <- state_agg_path[i]/state_agg_path[2]
  state_agg_path[i] <- lapply(state_agg_path[i], function(x) percent(x))
}

fed_ethnicities <- c("All Students",
                     "American Indian/ Alaskan Native",
                     "Asian",
                     "Black/ African American", 
                     "Hispanic/ Latino of any race(s)",
...
long_state_agg_path_race <- state_agg_path %>%
  select("Student Group", all_of(plotting_pathways)) %>%
  filter(`Student Group` %in% fed_ethnicities) %>%
...
long_state_agg_path_race %>%
  ...
  labs(x = "Student Groups", 
       y = "Percentage of Students Completed") +
  scale_y_continuous(labels = percent) +
  scale_fill_manual(name ="Pathways", values = palette_uw_reverse(7)) +
 # geom_text(aes(label = `% Completed` ), position = position_dodge(0.9),
#            hjust = -0.1, vjust = 0.4, colour = "black", size = 1) +
  scale_x_discrete(labels = function(x) str_wrap(str_replace_all(x, "foo" , " "),
                                                 width = 23)) +
  theme_uw() +
  coord_flip() +
...
#ggsave("/Users/alexliu/Desktop/A.5.2.i graduation pathway aggregate state total by race.png")
  
```

```{r *A.5.2.ii graduation pathway, aggregate state total, by income}
income_group <- c("All Students",
                  "Low-Income",
                  "Non-Low Income")
#change in selection of pathways
plotting_pathways <- factor(c("ELA and Math Both",
...
#ggsave("/Users/alexliu/Desktop/A.5.2.ii graduation pathway aggregate state total by income.png")
```

```{r A.5.2.iii graduation pathway agg state total w nopath by race}
fed_ethnicities <- c("All Students",
                     "American Indian/ Alaskan Native",
                     "Asian",
                     "Black/ African American", 
...
long_state_agg_path_race %>%
  ...
  labs(x = "Student Groups", 
       y = "Percentage of Students Completed") +
  scale_y_continuous(labels = percent, limits = c(0, 1)) +
  scale_fill_manual(name ="Pathways", values = palette_uw_reverse(8)) +
  geom_text(aes(label = `% Completed` ), position = position_dodge(0.9),
            hjust = -0.1, vjust = 0.4, colour = "black", size = 1) +
  theme_uw() +
  coord_flip() +
...
#ggsave("/Users/alexliu/Desktop/A.5.2.iii graduation pathway agg state total w nopath by race.png")
```

```{r A.5.2.iv graduation pathway agg state total w nopath by inc}
