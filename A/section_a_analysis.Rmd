...
course_by_race %>%
  ...
  labs(x = "Student Groups", 
       y = "All Non-Passing Grades",
       caption = "Note: change is calculated by percent in SY2020-21 minus percent in SY2018-19") +
  scale_y_continuous(labels = percent, limits = c(-0.005, 0.12)) +
  scale_fill_manual(values = uw_palette(5)) +
  geom_text(aes(label = `Change`), position = position_dodge(0.9),
            hjust = 0, vjust = 0.25, colour = "black", size = 1.5) +
  theme_light() +
  coord_flip() +
...
df_state_sqss_regattend <- df_sqss %>%
  filter(DistrictName == "State Total") %>%
...
```{r A.4.3.i Dual credit state total by income}
