...
df_s275 %>% 
  ...
  geom_text(aes(label = round(val*100, digits = 1)), position = position_dodge(width = 0.8), vjust = -0.25, size = 3) +
  ...
  scale_fill_manual(values = c("#4b2e83", "#8F7CB2", "#E9E5F0", "#85754d", "#B3A990"),
                    limits = c("1", "1.06", "1.12", "1.18", "1.24")) +
  theme_uw() +
  theme(axis.line = element_line()) +
...
df_s275 %>% 
  ...
  theme_uw() +
  theme(axis.line = element_line(), panel.grid.major.x = element_blank(), panel.grid.minor.x = element_blank()) +
...
df_s275 %>% 
  ...
  geom_text(aes(label = if_else(dis_name %in% outlier_disnames, as.character(dis_name), '')),
            hjust=0, vjust=0, size = 3, position = position_jitter(width = 0.01, height = 0, seed = 99)) +
  geom_point(aes(size = dist_enroll), alpha = 0.4, position = position_jitter(width = 0.01, height = 0, seed = 99)) +
  geom_smooth(method = "lm", aes(weight = dist_enroll), se = FALSE) +
  scale_x_continuous("Regionalization Factor", limits = c(.99, 1.28), breaks = c(1.00, 1.06, 1.12, 1.18, 1.24)) +
  scale_y_continuous("Average District Teacher Salary", limits = c(30000, 100000), labels=scales::dollar_format()) +
  ...
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  labs(title = "Scatterplot of (unadjusted) teacher base salary and regionalization factor \nfor school districts during AY 2018-2019",
       caption = "Fitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
df_s275 %>% 
  ...
  scale_color_manual(name='Teacher race', values = c('Black'='#4B2E83', 'Hispanic'='#A596C1', 'Asian'='#C2BAA6', 'Non-white'='#85754D')) +
  scale_y_continuous(labels = scales::percent) +
  theme_uw() +
  theme(axis.line = element_line(), panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  labs(y =  "",
       x = "",
       title = "Share of non-white teachers across WA school districts\nbetween AY 2015-2016 to 2020-2021")
...
df_s275 %>% 
  ...
  mutate(dist_pctstu_nonwhite = 1 - mean(dis_pctstu_white, na.rm = TRUE) - mean(dis_pctstu_asian, na.rm = TRUE),
         dist_pctstu_frl = 1 - mean(dis_pctstu_frl, na.rm = TRUE) - mean(dis_pctstu_frl, na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(exp_bin = if_else(exp <= 3, "early-career",
         ...
         dist_poverty_cat = if_else(dist_pctstu_frl < 0.26, "0 to 25%",
                                    if_else(dist_pctstu_frl >= 0.26 & dist_pctstu_frl < 0.51, "26 to 50%",
                                            if_else(dist_pctstu_frl >= 0.51 & dist_pctstu_frl < 0.76, "51 to 75%",
                                                    if_else(dist_pctstu_frl >= 0.76, "76 to 100%", "FLAG"))))) %>% 
  filter(!is.na(dist_nonwhite_cat) & !is.na(dist_poverty_cat)) %>% 
  group_by(exp_bin) %>% 
  summarize(share_black = mean(race == "B" & hispanic == "N", na.rm = TRUE),
            ...
            share_nonwhite = mean(race != "W" | (race == "W" & hispanic == "Y"), na.rm = TRUE),
            num_nonwhite = sum(race != "W" | (race == "W" & hispanic == "Y"), na.rm = TRUE)) %>% 
  ...
  scale_fill_manual("",
                    values = c("#4B2E83", "#8F7CB2", "#85754D", "#B3A990"),
                    breaks = c("black", "asian", "hisp", "nonwhite"),
                    labels = c("Black", "Asian", "Hispanic", "Non-white")) +
  scale_y_continuous(limits = c(0, .2), labels = scales::percent) +
  theme_uw() +
  theme(axis.line = element_line(), panel.grid.major.x = element_blank()) +
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  labs(x = "", y = "",
       title = "Share of non-white teachers by experience in WA state during AY 2018-2019",
       caption = "experience categories: early-career (0−3 years of teaching experience); junior (4−7); mid-career 1 (8−11); mid-career 2 (12−15);\n  late-career 1 (16−19); late-career 2 (20−22); and late-career 3 (23 and up) above).\nNumber of teachers in each category are also reported on top of each bar.")
...
df_s275 %>% 
  ...
  summarize(share_black = mean(race == "B" & hispanic == "N", na.rm = TRUE),
            ...
            num_white = sum(race == "W" & hispanic == "N", na.rm = TRUE),
            share_nonwhite = mean(race != "W" | (race == "W" & hispanic == "Y"), na.rm = TRUE),
            num_nonwhite = sum(race != "W" | (race == "W" & hispanic == "Y"), na.rm = TRUE)) %>% 
  pivot_longer(cols = c(starts_with("share_"), starts_with("num_")), names_to = c(".value", "race"), names_pattern = "(.*)_(.*)") %>% 
  ggplot(aes(x = exp_bin, y = share, fill = race)) +
  ...
  geom_text(aes(label = num), position = position_dodge(width = 0.9), vjust = -0.5) +
  scale_fill_manual("",
                    values = c("#4B2E83", "#8F7CB2", "#85754D", "#B3A990", "#E9E5F0"),
                    breaks = c("black", "asian", "hisp", "nonwhite", "white"),
  ...
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  labs(x = "", y = "",
       title = "Share of teachers by experience and race in WA state during AY 2018-2019",
       caption = "experience categories: early-career (0−3 years of teaching experience); junior (4−7); mid-career 1 (8−11); mid-career 2 (12−15);\n  late-career 1 (16−19); late-career 2 (20−22); and late-career 3 (23 and up) above).\nNumber of teachers in each category are also reported on top of each bar.")
...
df_s275 %>% 
  ...
  filter(!is.na(dist_nonwhite_cat) & !is.na(dist_poverty_cat)) %>% 
  group_by(exp_bin, dist_nonwhite_cat) %>% 
  summarize(share_black = mean(race == "B" & hispanic == "N", na.rm = TRUE),
            num_black = sum(race == "B" & hispanic == "N", na.rm = TRUE),
            share_asian = mean(race == "A" & hispanic == "N", na.rm = TRUE),
            num_asian = sum(race == "A" & hispanic == "N", na.rm = TRUE),
  ...
  ggplot(aes(x = exp_bin, y = share, fill = race)) +
  geom_col(position = 'dodge') +
  geom_text(aes(label = num), position = position_dodge(width = 0.9), vjust = -0.5, size = 2.5) +
  scale_fill_manual("",
                    values = c("#4B2E83", "#8F7CB2", "#85754D", "#B3A990"),
                    breaks = c("black", "asian", "hisp", "nonwhite"),
                    labels = c("Black", "Asian", "Hispanic", "Non-white")) +
  ...
  theme_uw() +
  theme(axis.line = element_line(), panel.grid.major.x = element_blank()) +
  theme(plot.caption = element_text(hjust = 0, face= "italic"), axis.text.x = element_text(angle = 45)) +
  facet_wrap(~ dist_nonwhite_cat) +
  labs(x = "", y = "",
       title = "Share of non-white teachers by experience and share of students of color\nin WA state during AY 2018-2019",
       caption = "experience categories: early-career (0−3 years of teaching experience); junior (4−7); mid-career 1 (8−11); mid-career 2 (12−15);\n  late-career 1 (16−19); late-career 2 (20−22); and late-career 3 (23 and up) above).\nNumber of teachers in each category are also reported on top of each bar.\nTop left panel shows share of non-white teachers for school districts with 0-25% students of color.")
...
df_s275 %>% 
  ...
  facet_wrap(~ dist_nonwhite_cat) +
  labs(x = "", y = "",
       title = "Share of teachers by race, experience and share of students of color\nin WA state during AY 2018-2019",
       caption = "experience categories: early-career (0−3 years of teaching experience); junior (4−7); mid-career 1 (8−11); mid-career 2 (12−15);\n  late-career 1 (16−19); late-career 2 (20−22); and late-career 3 (23 and up) above).\nNumber of teachers in each category are also reported on top of each bar.\nTop left panel shows share of teachers for school districts with 0-25% students of color.")
...
df_s275 %>% 
  ...
  filter(!is.na(dist_nonwhite_cat) & !is.na(dist_poverty_cat)) %>% 
  group_by(exp_bin, dist_poverty_cat) %>% 
  summarize(share_black = mean(race == "B" & hispanic == "N", na.rm = TRUE),
            num_black = sum(race == "B" & hispanic == "N", na.rm = TRUE),
            share_asian = mean(race == "A" & hispanic == "N", na.rm = TRUE),
            num_asian = sum(race == "A" & hispanic == "N", na.rm = TRUE),
  ...
  theme(plot.caption = element_text(hjust = 0, face= "italic"), axis.text.x = element_text(angle = 45)) +
  labs(x = "", y = "",
       title = "Share of non-white teachers by experience and share of students poverty (measured by FRPL)\nin WA state during AY 2018-2019",
       caption = "experience categories: early-career (0−3 years of teaching experience); junior (4−7); mid-career 1 (8−11); mid-career 2 (12−15);\n  late-career 1 (16−19); late-career 2 (20−22); and late-career 3 (23 and up) above).\nNumber of teachers in each category are also reported on top of each bar.\nTop left panel shows share of non-white teachers for school districts with 0-25% students poverty (measured by FRPL)")
...
df_s275 %>% 
  ...
  theme(plot.caption = element_text(hjust = 0, face= "italic"), axis.text.x = element_text(angle = 45)) +
  labs(x = "", y = "",
       title = "Share of teachers by race, experience and share of students poverty (measured by FRPL)\nin WA state during AY 2018-2019",
       caption = "experience categories: early-career (0−3 years of teaching experience); junior (4−7); mid-career 1 (8−11); mid-career 2 (12−15);\n  late-career 1 (16−19); late-career 2 (20−22); and late-career 3 (23 and up) above).\nNumber of teachers in each category are also reported on top of each bar.\nTop left panel shows share of teachers for school districts with 0-25% students poverty (measured by FRPL)")
...
df_s275_dist <- df_s275 %>% 
  filter(SchoolYear == "2018-2019" & !is.na(rfactor_cis_2019)) %>% 
  mutate(rfactor = as.character(rfactor_cis_2019)) %>% 
  group_by(coudis_s, rfactor, dis_name) %>% 
...
temp <- df_s275_dist %>% 
  filter((dist_pctstu_nonwhite < 0.15 & mean_dist_tch_nonwhite > 0.12) | (dist_pctstu_nonwhite  > 0.72))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>% 
  ggplot(aes(x = dist_pctstu_nonwhite, y = mean_dist_tch_nonwhite)) +
  geom_point(aes(size = dist_enroll), alpha = 0.5) +
  geom_text(aes(label = if_else(dis_name %in% outlier_disnames, as.character(dis_name), '')),
  ...
  labs(x = "Share of students of color",
       ...
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")


temp <- df_s275_dist %>% 
  filter((dist_pctstu_black < 0.05 & mean_dist_tch_black > 0.027) | (dist_pctstu_black  > 0.15))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>% 
...
temp <- df_s275_dist %>% 
  filter((dist_pctstu_hisp < 0.25 & mean_dist_tch_hisp > 0.15) | (dist_pctstu_hisp  > 0.8))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
...
temp <- df_s275_dist %>% 
  filter((dist_pctstu_asian < 0.1 & mean_dist_tch_asian > 0.05) | (dist_pctstu_asian < 0.1 & dist_pctstu_asian > 0.08 & mean_dist_tch_asian < 0.015 & mean_dist_tch_asian > 0) | (dist_pctstu_asian  > 0.25))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>% 
...
temp <- df_s275_dist %>% 
  filter((dist_pctstu_white < 0.18 & mean_dist_tch_white > 0.87 & mean_dist_tch_white < 0.95) | (dist_pctstu_white < 0.10 & mean_dist_tch_white > 0.95) | 
           (dist_pctstu_white > 0.23 & dist_pctstu_white < 0.25 & mean_dist_tch_white > 0.87) | 
           (dist_pctstu_white < 0.25 & mean_dist_tch_white < 0.6) | (dist_pctstu_white  > 0.8 & mean_dist_tch_white < 0.8))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
...
df_s275_dist %>%
  filter(!is.na(dist_nonwhite_cat)) %>% 
  ...
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  scale_color_manual("",
                    values = c("#4B2E83", "#A596C1", "#85754D", "#C2BAA6")) +
...
temp <- df_s275_dist %>% 
  filter(mean_dist_tch_nonwhite1 > 0.33 | (dist_pctstu_nonwhite1 > 0.75 & dist_pctstu_nonwhite1 < 0.9 & mean_dist_tch_nonwhite1 < 0.05) | 
           (dist_pctstu_nonwhite1 > 0.9 & mean_dist_tch_nonwhite1 > 0.1 & mean_dist_tch_nonwhite1 < 0.25) | (dist_pctstu_nonwhite1 > 0.9 & mean_dist_tch_nonwhite1 > 0.35))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
...
temp <- df_s275_dist %>% 
  filter((dist_pctstu_frpl < 0.25 & mean_dist_tch_nonwhite > 0.1) | (dist_pctstu_frpl < 0.75 & dist_pctstu_frpl > 0.50 & mean_dist_tch_nonwhite > 0.3) | (dist_pctstu_frpl > 0.85))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>% 
  ...
  labs(x = "Share of students receiving FRPL",
       y = "Share of non-white teachers",
       title = "Scatterplot of share of nonwhite teachers and students poverty (measured by FRPL)\nacross WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
# bachelor
temp <- df_s275_dist %>% 
  filter((dist_pctstu_nonwhite < 0.25 & mean_dist_tch_bachelor > 0.5) | (dist_pctstu_nonwhite > 0.75))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>%
...
df_s275_dist %>%
  ...
  labs(x =  "Share of students of color in a district",
       ...
       title = "Scatterplot of share of teachers with highest degree as master's and share of students of color\nacross WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
df_s275_dist %>%
  ...
  labs(x =  "Share of students of color in a district",
       ...
       title = "Scatterplot of share of teachers with highest degree as doctorate's and share of students of color\nacross WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
# master or doctorate
temp <- df_s275_dist %>% 
  filter((dist_pctstu_nonwhite < 0.25 & mean_dist_tch_master_plus > 0.83) | (dist_pctstu_nonwhite < 0.25 & mean_dist_tch_master_plus < 0.5) | (dist_pctstu_nonwhite > 0.75))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>%
  ...
  labs(x =  "Share of students of color in a district",
       ...
       title = "Scatterplot of share of teachers with highest degree as master's or doctorate and\nshare of students of color across WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
df_s275_dist %>%
  ...
  labs(x =  "Share of teachers with master's/doctorate",
       y = "",
       color = "Share of students of\ncolor in a district",
       title = "Density of share of teachers with highest degree as master's or doctorate\nacross WA school districts in AY 2018-2019 by low to high shares of students of color",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.")
...
df_s275_dist %>%
  ...
  labs(x =  "Share of students of color in a district",
       ...
       title = "Scatterplot of share of teachers with no bachelor's and share of students of color\nacross WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
# bachelor
temp <- df_s275_dist %>% 
  filter((dist_pctstu_frpl < 0.24 & mean_dist_tch_bachelor > 0.5) | (dist_pctstu_frpl < 0.24 & mean_dist_tch_bachelor < 0.2) | 
          (dist_pctstu_frpl > 0.9 & mean_dist_tch_bachelor > 0.5) | (dist_pctstu_frpl > 0.75 & mean_dist_tch_bachelor < 0.2))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>%
  ...
  labs(x =  "Share of students receiving FRPL",
       y = "Share of teachers with bachelor's",
       title = "Scatterplot of share of teachers with highest degree as bachelor's and\nshare of students poverty (measured by FRPL) across WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")

df_s275_dist %>%
  filter(!is.na(dist_poverty_cat)) %>% 
  ggplot(aes(x = mean_dist_tch_bachelor, color = dist_poverty_cat)) +
  geom_density(aes(y=..scaled..)) + 
  xlim(0, 0.9) +
  theme_uw() +
  theme(axis.line = element_line()) +
...
# master
df_s275_dist %>%
  ggplot(aes(x = dist_pctstu_frpl, y = mean_dist_tch_master)) +
  geom_point(aes(size = dist_enroll)) + 
  geom_smooth(method = "lm", aes(weight = dist_enroll), se = FALSE) +
  theme_uw() +
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  ...
  labs(x =  "Share of students receiving FRPL",
       y = "Share of teachers with master's",
       title = "Scatterplot of share of teachers with highest degree as master's and\nshare of students poverty (measured by FRPL) across WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")

df_s275_dist %>%
  filter(!is.na(dist_poverty_cat)) %>% 
  ggplot(aes(x = mean_dist_tch_master, color = dist_poverty_cat)) +
  geom_density(aes(y=..scaled..)) + 
  xlim(0, 1) +
  theme_uw() +
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
...
# master or doctorate
temp <- df_s275_dist %>% 
  filter((dist_pctstu_frpl < 0.25 & mean_dist_tch_master_plus > 0.77) | (dist_pctstu_frpl < 0.25 & mean_dist_tch_master_plus < 0.5) | 
          (dist_pctstu_frpl > 0.9 & mean_dist_tch_master_plus > 0.75) | (dist_pctstu_frpl > 0.9 & mean_dist_tch_master_plus < 0.5))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>%
  ggplot(aes(x = dist_pctstu_frpl, y = mean_dist_tch_master_plus)) +
  geom_point(aes(size = dist_enroll), alpha = 0.5) + 
  geom_text(aes(label = if_else(dis_name %in% outlier_disnames, as.character(dis_name), '')),
            hjust=0, vjust=0, size = 3) +
  geom_smooth(method = "lm", aes(weight = dist_enroll), se = FALSE) +
...
df_s275_dist %>%
  filter(!is.na(dist_poverty_cat)) %>% 
  ggplot(aes(x = mean_dist_tch_master_plus, color = dist_poverty_cat)) +
  geom_density(aes(y=..scaled..)) + 
  xlim(0, 1) +
  theme_uw() +
  theme(axis.line = element_line()) +
  ...
  labs(x =  "Share of teachers with master's/doctorate",
       y = "",
       color = "Share of students\nreceiving FRPL",
       title = "Density of share of teachers with highest degree as master's or doctorate across WA\nschool districts in AY 2018-2019 by low to high shares of students poverty (measured by FRPL)",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.")
...
df_s275_dist %>%
  ...
  labs(x =  "Share of students receiving FRPL",
       y = "Share of teachers with no bachelor's",
       title = "Scatterplot of share of teachers with no bachelor's and\nshare of students poverty (measured by FRPL) across WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
temp <- df_s275_dist %>% 
  filter((dist_pctstu_nonwhite < 0.25 & mean_dist_tch_exp > 18) | (dist_pctstu_nonwhite < 0.5 & mean_dist_tch_exp < 10) | 
          (dist_pctstu_nonwhite > 0.5 & mean_dist_tch_exp > 16) | dist_pctstu_nonwhite > 0.9)
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>%
  ...
  labs(x =  "Share of students of color",
       ...
       title = "Scatterplot of average teachers experience and share of students of color\nacross WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
temp <- df_s275_dist %>% 
  filter((dist_pctstu_frpl < 0.25 & mean_dist_tch_exp > 18) | (dist_pctstu_frpl < 0.5 & mean_dist_tch_exp < 10) | 
          (dist_pctstu_frpl > 0.75 & mean_dist_tch_exp > 18) | (dist_pctstu_frpl > 0.9 & mean_dist_tch_exp < 10))
outlier_disnames <- c(as.vector(as.matrix(temp[, c("dis_name", "dis_name")])),
                      "Seattle Public Schools", "Lake Washington School District",
                      "Spokane School District", "Tacoma School District")
df_s275_dist %>%
  ...
  labs(x =  "Share of students receiving FRPL",
       y = "Average teacher experience (in years)",
       title = "Scatterplot of average teachers experience and share of students poverty (measured by FRPL)\nacross WA school districts in AY 2018-2019",
       caption = "In all estimates, asians are excluded from non-white or people of color categories.\nFitted line is weighted by district's enrollment size. The size of each dot represents the dirtrict's enrollment size.")
...
df_state_level <- df_s275_turnover %>% 
  group_by(schyear) %>% 
  summarize(tch_leavers = mean(tch_turnover == "leave"),
            tch_movers = mean(tch_turnover == "diff_dist_tch"),
            tch_newcomer1 = mean(tch_newcomer1 == 1),
            tch_newcomer2 = mean(tch_newcomer2 == 1))
...
df_s275_turnover %>% 
  ...
  geom_line(data = subset(df_state_level, schyear < 2021), aes(x = schyear, y = tch_leavers, color = "State Average", linetype = "Leavers")) +
  geom_point(data = subset(df_state_level, schyear < 2021), aes(x = schyear, y = tch_leavers, color = "State Average")) +
  geom_line(aes(y = tch_movers, linetype = "Movers")) + 
  geom_point(aes(y = tch_movers, color = dist_nonwhite_cat)) +
  geom_line(data = subset(df_state_level, schyear < 2021), aes(x = schyear, y = tch_movers, color = "State Average", linetype = "Movers")) +
  geom_point(data = subset(df_state_level, schyear < 2021), aes(x = schyear, y = tch_movers, color = "State Average")) +
  ylim(0, 0.15) +
  theme_uw() +
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  scale_color_manual("",
                     values = c("#4B2E83", "#85754D", "#C2BAA6")) +
...
df_s275_turnover %>% 
  ...
  geom_line() +
  geom_point() +
  geom_line(data = subset(df_state_level,schyear > 2016 & schyear <= 2021), aes(x = schyear, y = tch_newcomer1, color = "State Average")) +
  geom_point(data = subset(df_state_level,schyear > 2016 & schyear <= 2021), aes(x = schyear, y = tch_newcomer1, color = "State Average")) +
  ylim(0, .2) +
  theme_uw() +
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  scale_color_manual("",
...
labs(x = "",
     ...
     caption = "*newcomer is defined as a teacher who was not present in the system in the prior year.\nIn all estimates, asians are excluded from non-white or people of color categories.\n")
...
df_s275_turnover %>% 
  ...
  geom_line() +
  geom_point() +
  geom_line(data = subset(df_state_level,schyear > 2016 & schyear <= 2021), aes(x = schyear, y = tch_newcomer2, color = "State Average")) +
  geom_point(data = subset(df_state_level,schyear > 2016 & schyear <= 2021), aes(x = schyear, y = tch_newcomer2, color = "State Average")) +
  ylim(0, .2) +
  theme_uw() +
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
  scale_color_manual("",
  ...
  labs(x = "",
       ...
       color = "Share of students\nof color",
       title = "Average teacher newcomers* rate by high vs. low students of color in WA state\nbetween AY 2016-2017 to 2020-2021",
       caption = "*newcomer is defined as a teacher who was not present in his/her current school district in the prior year.\nIn all estimates, asians are excluded from non-white or people of color categories.")
