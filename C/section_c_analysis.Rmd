...
df_s275 %>% 
  ...
  geom_smooth(method='lm', se = FALSE) + 
  geom_smooth(method='lm', aes(weight = dist_enroll), color = "red", se = FALSE) +
  scale_y_continuous("Average District Teacher Salary", limits = c(30000, 100000), labels=scales::dollar_format()) +
...
df_s275 %>% 
  ...
  pivot_longer(cols = starts_with("share_"), names_to = "race", values_to = "val") %>% 
  ggplot(aes(x = exp_bin, y = val, fill = race)) +
  geom_col(position = 'dodge') +
  scale_fill_discrete("",breaks = c("share_black", "share_asian", "share_hisp", "share_nonwhite", "share_nonwhite1"),
...
df_s275_dist <- df_s275 %>% 
  ...
  summarize(mean_dist_tch_black = mean(race == "B", na.rm = TRUE),
            ...
            dist_pctstu_nonwhite1 = 1 - dist_pctstu_white - dist_pctstu_asian,
...
df_s275_turnover %>% 
  ...
  labs(x = "",
       y = "newcomers rate",
       color = "Share of students\nof color",
       title = "Average teacher newcomers rate by high vs. low students of color in WA state\nbetween AY 2016-2017 to 2018-2019",
