...
#Projected amounts for 2019, does not include 2020
...
#OLD VERSION OF CWI INCORPORATED INFLATION WHEN NEW DOESN
#DIVIDE BY RATIO OF CWI FOR THAT DISTRICT
#CWI/STATEWIDE AVERAGE FOR EACH YEAR, MAKE SURE EACH YEAR'S CWI IS ONE
#USE AN ACADEMIC CPI
...
model_fed <- lm(rev_pp_cwi_18d ~ log(TotalEnrollment) + LOCALE + PctSpecialEducation + PctTransitionalBilingual  + PctFreeorReducedPricedMeals + schyear, data=df_f195_fed)

model_fed_nocpi <- lm(rev_pp_cwi ~ log(TotalEnrollment) + LOCALE + PctSpecialEducation + PctTransitionalBilingual + PctFreeorReducedPricedMeals + schyear, data=df_f195_fed)

df_f195_fed$rev_adjusted <- predict.lm(model_fed, newdata=df_f195_fed)
df_f195_fed$rev_adjusted_nocpi <- predict.lm(model_fed_nocpi, newdata=df_f195_fed)

fed_margins <- margins(model_fed, at = list(PctFreeorReducedPricedMeals = 0.1:0.9))
...
model_state <- lm(rev_pp_cwi_18d ~ log(TotalEnrollment) + LOCALE + PctSpecialEducation + PctTransitionalBilingual + PctFreeorReducedPricedMeals + schyear, data=df_f195_state)

model_state_nocpi <- lm(rev_pp_cwi ~ log(TotalEnrollment) + LOCALE + PctSpecialEducation + PctTransitionalBilingual + PctFreeorReducedPricedMeals + schyear, data=df_f195_state)

df_f195_state$rev_adjusted <- predict.lm(model_state, newdata=df_f195_state)
df_f195_state$rev_adjusted_nocpi <- predict.lm(model_state_nocpi, newdata=df_f195_state)

#pred_prob_state <- ggpredict(model_state, "PctFreeorReducedPricedMeals") #Marginal effect and predictions

#LOCAL
df_f195_loc <- df_f195_merge%>%
  filter(rev_source=="Local")

model_loc <- lm(rev_pp_cwi_18d ~ log(TotalEnrollment) + LOCALE + PctSpecialEducation + PctTransitionalBilingual + PctFreeorReducedPricedMeals + schyear, data=df_f195_loc)

model_loc_nocpi <- lm(rev_pp_cwi ~ log(TotalEnrollment) + LOCALE + PctSpecialEducation + PctTransitionalBilingual + PctFreeorReducedPricedMeals + schyear, data=df_f195_loc)

df_f195_loc$rev_adjusted <- predict.lm(model_loc, newdata=df_f195_loc)
df_f195_loc$rev_adjusted_nocpi <- predict.lm(model_state_nocpi, newdata=df_f195_loc)

pred_prob_loc <- ggpredict(model_loc, "PctFreeorReducedPricedMeals") #Marginal effect and predictions
...
df_f195_total <- data.frame(
  ...
  df_f195_fed$rev_adjusted_nocpi,
  df_f195_state$rev_adjusted,
  df_f195_state$rev_adjusted_nocpi,
  df_f195_loc$rev_adjusted,
  df_f195_loc$rev_adjusted_nocpi)
...
df_f195_total <- df_f195_total%>%
  mutate(rev_adjusted = df_f195_fed.rev_adjusted + df_f195_state.rev_adjusted + df_f195_loc.rev_adjusted,
         rev_adjusted_nocpi = df_f195_fed.rev_adjusted_nocpi + df_f195_state.rev_adjusted_nocpi + df_f195_loc.rev_adjusted_nocpi)%>%
  select(-c(df_f195_fed.rev_adjusted, df_f195_state.rev_adjusted, df_f195_loc.rev_adjusted, df_f195_fed.rev_adjusted_nocpi, df_f195_state.rev_adjusted_nocpi, df_f195_loc.rev_adjusted_nocpi))%>%
...
ggplot(df_f195_fed, 
       aes(x=schyear, y=rev_adjusted, fill=dist_group_soc)) + 
  stat_summary(geom="line", fun="mean", alpha=0.5) +
  stat_summary(geom="ribbon", 
               fun.data = mean_se, #mean_cl_normal
...
wa_df196 %>%
  ...
  scale_fill_gradient2(low="#FFFFFF", high="#4b2e83", name="2018 Dollars") +
  labs(title="Local Levy Revenue Per Pupil, 2018-2019")+
  theme_uw_map()
