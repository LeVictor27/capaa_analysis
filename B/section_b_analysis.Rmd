...
df_f196_merge<- full_join(df_f196, df_cwi, by=c("coudis"="dist_id", "schyear"="year"))
...
ref <- df_f196_merge%>%
  select(schyear, leaid, dist_name, rev_st_18d_Total, localrev_201819Spring_18d, dis_enroll)%>%
  filter(schyear==2019)%>%
  mutate(leaid = as.character(leaid),
         rev_st_pp_18d=(rev_st_18d_Total/dis_enroll),
         rev_loc_pp_18d=(localrev_201819Spring_18d/dis_enroll))
...
wa_df196 %>%
  ...
  scale_fill_gradient(low = "white", high="#4b2e83", name="2018 Dollars") +
  theme_void() +
